...
while True:
    ...
    if now.hour >= 8 and now.hour <= 23:
            # Run the command without storing the output
            subprocess.run(['echo', '255', '|', 'sudo', 'tee', '/sys/class/leds/led1/brightness'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            ...
            draw.text((x, top), "Raspberry Pi Diego", font=font, fill=255)      #Tu texto personalizado 1
            ...
            subprocess.run(['echo', '0', '|', 'sudo', 'tee', '/sys/class/leds/led1/brightness'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    # Muestra imagen.
    disp.image(image)
    disp.show()
